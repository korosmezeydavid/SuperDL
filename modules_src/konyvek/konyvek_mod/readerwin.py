...
class ReaderFrame(wx.Frame):
    ...
    def _build(self):
        ...
        self.clean_chk = wx.CheckBox(p, label="Felesleges sorok &tisztítása")
        self.clean_chk.SetName(
            "A felolvasás előtt összevonja a kettévágott szavakat és a kemény "
            "sortöréseket, és kihagyja az üres sorokat és oldalszámokat")
        self.clean_chk.SetValue(True)
        self.clean_chk.Bind(wx.EVT_CHECKBOX, lambda e: self._reapply_clean())
        top.Add(b_open, 0, wx.RIGHT, 8)
        top.Add(self.clean_chk, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)
    ...
    def _reapply_clean(self):
        """A tisztítás-kapcsoló átállításakor a megnyitott könyvet újratölti a
        nyers szövegből (a kijelzés, a motor és a könyvjelző együtt marad)."""
        raw = getattr(self, "_raw_text", None)
        if raw is not None:
    ...
    def _set_book(self, path, text, title):
        ...
        self._raw_text = text                    # nyers szöveg a kapcsoló újra-alkalmazásához
        if self.clean_chk.GetValue():
            text = audiobook.clean_for_speech(text)
