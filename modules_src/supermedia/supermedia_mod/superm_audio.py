...
def _dll_path() -> str:
    ...
    if mp:                                  # fagyasztott app: a DLL ide bundle-ölve
        cands.append(Path(mp) / "bass.dll")
    # MODULBÓL futva a saját mappánk NEM a Core bin-je: a Core `superdl`
    # csomagjához horgonyzunk, így a bass.dll mindig a Core bin/-jéből jön.
    try:
        import superdl
        cands.append(Path(superdl.__file__).resolve().parent.parent
                     / "bin" / "bass.dll")
    except Exception:
        pass
