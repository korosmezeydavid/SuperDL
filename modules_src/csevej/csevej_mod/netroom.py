...
SDK-függőség: csak a Core-ban már meglévő `requests`. Nincs központi szerver,
nincs VPS – az Ably ingyenes rétege továbbít, semmit nem tárolunk tartósan.

A kulcs sosem kerül a nyilvános forrásba: env `SUPERDL_ABLY_KEY`, a fejlesztő
helyi kulcsa (~/.superdl/ably_key.txt), vagy a modulba CSOMAGOLT közös kulcs
(git-ignorált `ably_kulcs_beepitett.txt`, csak a kiadott ZIP-ben) – így a
végfelhasználónak CSAK internet kell, semmit nem kell beállítania.

Ez a fájl a Szerencsekerék `netroom`-jának általánosított testvére: a csatorna-
előtag paraméterezhető (itt „csevej”), így a két funkció nem ütközik.
"""
import json
import os
import random
...
def ably_kulcs() -> str:
    ...
    modulba CSOMAGOLT közös kulcs. Az utóbbi teszi lehetővé, hogy MINDEN
    felhasználó beállítás nélkül, csak internettel csevegjen."""
...
class NetSzoba:
    """Egy valós idejű szoba (egy Ably-csatorna). Nem tud a tartalomról semmit,
    csak típusos üzeneteket küld/fogad a szobában."""
